...
const GoogleAuth = () => {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      if (result.user) {
        const response = await fetch(`${backendHostLink}/auth/googleauth`, {
          method: "POST",
          headers: {
  ...
  return (
    <>
      <div className="text-[14px] text-neutral-400 py-2 w-full flex justify-center items-center gap-2">
        <div className="h-[1px] bg-neutral-600 w-full"></div>
        <div className="w-[400px] text-center">Or sign in with</div>
        <div className="h-[1px] bg-neutral-600 w-full"></div>
      </div>
